...
- Treat completed Runs as immutable. Keep simple observations inline in Evidence. Promote an optional Observation Record only when a condition in the exact versioned promotion-trigger Registry applies; never invent an unregistered trigger or use review as a generic escape hatch. A missing condition remains a proposal until independent review, explicit human adoption, and protected Registry maintenance make it effective. A finalized Observation binds the Registry, every source Run and exact analysis fingerprint, has no Claim assessment, and is never modified in place.
- Claims reference finalized Evidence; each new Evidence Argument addresses one Claim and either embeds its observation or pins one finalized Observation by `{observation_id, version, sha256}`. Evidence must state the observation-to-Claim reasoning bridge, auxiliary assumptions, competing explanations, and conditions that would overturn its assessment. A result summary alone is not a scientific argument.
- Treat every Run as exploratory by default. Create and freeze a minimal Confirmation Record only before promoting a result to a high-strength Claim; only new Runs bound to its exact Claim, candidate, protocol, evaluator, held-out conditions, and unused slots are confirmatory. All records for the same exact Claim version belong to one derived confirmation campaign. A later record requires terminal predecessor slots plus an explicit replication or corrective-supersession rationale and differences; campaign Evidence must disclose every record and attempt. Never relabel an exploratory or legacy Run as confirmatory.
...
- Exploratory Evidence may support `under_test` or scoped `partially_supported` Claims. A `numerically_supported` Claim requires complete current-campaign confirmatory Evidence with either workflow-observed fresh held-out conditions or an explicitly justified `not_applicable` condition; mixed Evidence must expose its exploratory and confirmatory parts and cannot hide omitted attempts from any Confirmation in the campaign.
...
- The implementer is not the final reviewer. Run final scientific review in a fresh top-level read-only session. The human owns the Verdict decisions; after an explicit instruction that selects or adopts the exact implementation decision, scientific decision, Claim scope, rationale, and conditions, a separate write-enabled Agent may prepare the decision-only input and invoke `studyctl verdict --agent-initiated --file ...`. Never infer acceptance from silence, a passing review, a generic “continue/finish”, or the Agent's own recommendation. Ask one bounded alignment batch only when a material Verdict field remains ambiguous.
