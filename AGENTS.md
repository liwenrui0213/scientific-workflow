...
- Begin research or execution only after `studyctl validate` and from generated `ACTIVE_CONTEXT.json`: it contains bounded Claim and Frontier locators (IDs, short previews, counts and hashes) plus path/hash/size selectors for the approved Brief, active formal artifacts, and latest Checkpoint. Read authoritative source sections by current question or ID; generated projections are never authority.
