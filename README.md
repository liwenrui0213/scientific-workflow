...
Run/Observation/Evidence/Cohort、Confirmation 工作、work、正式制品、失败方向和宿主变更路径只保留有限 locator
...
当前运行时只接受其随附的当前 Claims、Evidence 和 Checkpoint schema；
...
缺少 `EVIDENCE.sequence.json`、但 Evidence 记录仍满足当前 schema 的连续历史，
需显式运行
`python -m tools.studyctl migrate-evidence-sequence SC-NNNN`；它会检查连续版本
和 Checkpoint 水位，并明确标注迁移前删除历史无法由本地文件证明。
缺少 `CHECKPOINTS.sequence.json`、但 Checkpoint 链仍满足当前 schema 的历史，
同样需要显式运行
