...
认知图（为什么、知道了什么）:
EvidenceGap -> （需要时的 ExperimentIntent） -> Observation -> Evidence -> Claim

控制图（需要 PLAN 时，如何执行）:
ExperimentIntent 精确引用 -> ControlGraphSpec -> active PLAN
外部编排 / 带可选 --plan-node 的单次 studyctl run -> Run / Artifact

跨图桥接:
Run / Artifact -> provenance-bound interpretation -> Observation

一个逻辑账本族（发生史与完整性，不是第三张科学图）:
RUNS.ledger + OBSERVATIONS/EVIDENCE/CHECKPOINTS/GRAPH_RECORDS sequences
```

本框架的目标不是让流程替代科学判断，而是让代码、实验和结论之间保持可追溯、可复现、可审查。`studyctl` 只记录和验证事实，不自动发明科学结论；账本连续、控制完成或 Checkpoint 存在都不能生成科学支持。
...
- **Intent / Plan 分离**：需要持久化“为什么”时，`ExperimentIntent` 记录证据缺口和请求的观察；它的 `assessment_semantics` 可以为 `null`。Run 可用独立 `intent_binding` 只绑定 why，而不创建 PLAN。`ControlGraphSpec`/PLAN 只记录“如何”，不是每个 Intent 或 Run 的必备附件；一旦存在，Run 的 Intent 与 control binding 中的 finalized Intent 必须完全一致。
- **Progressive formalization**：只在科学语义、共享依赖、计算成本、可复现性或审查要求上升时创建最小必要的 `METHOD`、`PROTOCOL`、`EVALUATOR`。仅当形式化门要求，或确有并行依赖、多 Agent/worker 编排等控制复杂度时才创建 `PLAN`；普通单步探索不需要先画图。
- **Agent-owned control topology**：ControlGraph v2 不规定 DAG、节点类别或重试/迭代方法。Agent 在 Brief、profile、预算和安全边界内选择拓扑并把控制语义写入开放 specification；`studyctl` 只验证结构引用、版本和精确字节，不解释这些不透明语义，也不执行整张图。执行某个节点时，`--plan-node` 会把 Run v5 精确绑定到图、Intent、节点和节点规格；活动 PLAN 不会自动绑定普通 Run，后续验证会对 finalized 图和节点命令重放该绑定。
- **One logical ledger family**：`RUNS.ledger.json` 与 Observation、Evidence、Checkpoint、Graph Record 的 sequence 文件物理职责不同，但共同保存单调发生史、标识与完整性绑定。Observation sequence v2 与 Evidence sequence v3 还绑定全部 finalized 记录的计数和 inventory hash，可检测删除；sequence 与 finalized 记录还必须是只读、硬链接计数为 1 的普通文件。只允许通过显式命令前向恢复一个唯一可重建的中断 finalization。账本不判断 Claim。
- **Claim-to-Evidence**：简单观察直接内嵌在单 Claim Evidence 中；只有版本化 Registry 中的提升条件适用时才创建不可变 Observation Record。新条件须经独立审查、明确的人类采纳和受保护的追加式 Registry 更新；Observation v3 绑定 Registry、源 Runs，以及由每个 Run 的独立 `intent_binding` 派生的精确 finalized Intent refs。Evidence v5 再绑定同一 Run-derived Intent 集合与所针对 Claim 的 statement/scope digest，并说明观察如何影响 Claim、依赖哪些辅助假设、有哪些竞争解释以及什么结果会推翻当前判断。全量验证会重放 finalized Evidence 的确定性 epistemic-basis 检查，但不会替人判断科学推理是否正确。
- **Archived Claim continuity**：新 Evidence 仍只能针对当前 active Claim；当合法压缩已把 terminal Claim 从当前 `CLAIMS.json` 移除时，旧 Evidence 的语义绑定改由 Checkpoint 精确固定的只读、内容寻址 Claim record 持续重放。
- **Failure layers stay separate**：失败事实由不可变 Run 记录；失败原因是待检验的候选解释；可复用经验必须写成有范围、由 Claim-specific Evidence 支持的 Claim。自由文本的“方向失败”笔记不是准正式权威。
...
- **Checkpoint is derived**：Checkpoint v5 是对源 Runs、Observations、Evidence、Claims、Frontier、正式制品和账本绑定的不可变、可追溯压缩，并 pin 当时的 graph-record-sequence locator，服务于有限恢复与精确审查范围；它不是新的科学真相来源。
...
`EvidenceGap ->（在需要形式化 why 时的 ExperimentIntent）-> Observation ->
Evidence -> Claim`。控制图只在需要结构化 how 时出现，包含
`ExperimentIntent` 精确引用、`ControlGraphSpec` 和活动 PLAN；实际
Run/Artifact 由 Agent、外部编排器或单次 `studyctl run` 产生。Run/Artifact 是
执行和 provenance 事实，只有经过解释的 Observation 和 Claim-specific Evidence
才能影响 Claim。

`ExperimentIntent` 说明为什么执行、请求哪些观察，以及在确有依据时怎样评估它们。
v2 的 `assessment_semantics` 可以为 `null`；若填写，finalization 会冻结其精确
判据供审计，但不会自动把判据结果转换为 Claim 支持。只有形式化门要求 PLAN，
或确有并行依赖、多 worker 编排等控制复杂度时，才创建 `ControlGraphSpec`。
普通单步探索可以没有 Intent/PLAN；若需要保留 why，可以只向 Run 传
`--intent INTENT_ID --intent-version N`。任何存在的 Plan 都必须精确绑定 finalized
Intent，`formal/PLAN.json` 必须是通过 `plan-activate` 选中的
ControlGraphSpec 的逐字节物化。

ControlGraph v2 只固定最小结构与引用完整性，不固定 DAG、节点类别、边条件语义
或重试/迭代策略。Agent 可以在开放 specification/metadata 中表达这些选择；
`studyctl` 不解释它们，也不自动调度整张图。Agent 执行某个节点时可传
`--plan-node NODE_ID`；Run v5 随即绑定精确的 ControlGraph ID/version/hash、
finalized Intent ref、节点 ID 和 node-spec hash。活动 PLAN 本身不会隐式绑定
普通 Run，已绑定的一组 Runs 也不等于整图已完成。

Observation v3 和 Evidence v5 会从各自源 Run 的独立 `intent_binding` 派生并精确
保存 finalized Intent refs；无 Intent 的 Run 贡献空集合。若 Run 同时绑定控制节点，
其 control binding 中的 Intent 必须与独立绑定完全一致。Evidence v5 另绑定所针对
Claim 的 statement/scope digest，Claim 规格变化后不能把旧 draft 偷换解释。
这些都是 traceability binding，仍须由 Evidence 完成科学推理。

`RUNS.ledger.json`、`OBSERVATIONS.sequence.json`、
`EVIDENCE.sequence.json`、`CHECKPOINTS.sequence.json` 和
`GRAPH_RECORDS.sequence.json` 组成一个逻辑账本族。它们分别绑定 Run
身份/预算、Observation/Evidence 创建高水位与 finalized inventory、
Checkpoint 尾部以及 finalized graph 记录清单；这是一组本地发生史与完整性
边界，不是第三张认知图，也不能从 Plan、Run 或 sequence 成功直接更新 Claim。
...
哈希均由 `studyctl` 自动绑定；无需手写带哈希的 JSON。已导入的 independent
Review 与对应 Review Packet 会先归档为内容寻址、只读记录，Verdict 再绑定两者
的路径、大小和 SHA-256。若当时没有导入 Review，Verdict 必须写入显式 waiver，
不能让 Review 依据默默缺席。记录前应先提交已审查状态，并为当前 Brief、审批和
Claims 生成最新 Checkpoint。
...
自由探索
  ...
  -> 冻结候选、协议、评价器、held-out 条件、分析规则和结果契约
...
确认记录不是新的人工审批，只是一个运行前不可变的时间与哈希边界。每个 slot
还要冻结可接受的终止状态，以及必须产生的 Observation 或输出 Artifact。每个
confirmatory Run 必须完全匹配冻结的 Claim、候选、协议、评价器、输入、输出和 slot；
...
`numerically_supported`，直到新 Evidence 覆盖当前完整历史。该状态还要求非空、
有边界的 Claim scope、满足结果契约的 supporting confirmatory Run，以及可追溯的
Observation/Artifact；若存在 contradictory Evidence，必须给出
`resolved`/`scope_limited` 的 conflict disposition 和显式 synthesis。旧 Run 或 exploratory
...
`execution_boundary` 记录后端、版本、策略格式与哈希、环境变量及其哈希、环境
白名单、只读/可写/设备路径和输出 staging；缺少或内部不一致的强制边界不能进入
Evidence。后端会在 Run ID 与预算分配前进行能力
...
下面这些物理文件共同构成一个逻辑账本族，但各自只有窄的事实/完整性权限：
...
`OBSERVATIONS.sequence.json` 和 `EVIDENCE.sequence.json` 分别以只增高水位
记录 Observation 与 Evidence draft 的创建次数；发布失败会烧掉一次计数，删除
文件不会降低压缩压力。Observation sequence v2 与 Evidence sequence v3 还保存
全部 finalized records 的精确 count 和 inventory hash，因而删除、替换或未索引
finalization 会失败关闭。若且仅若中断窗口留下一个可由旧 inventory 唯一重建的
新 finalized record，可分别运行：
...
这两个命令只向前索引一个记录，不从残缺历史猜测，也不降低高水位。
...
这些 ledger/sequence 能回答“记录是否发生、身份是否复用、当前清单是否一致”等
完整性问题，不能回答“结果是否支持 Claim”；科学支持仍只来自 Claim-specific
Evidence 的明示推理。
...
只保存路径、哈希、大小与计数摘要；Checkpoint 本身也是从其绑定的源记录派生的
压缩快照，不能替代源 Evidence/Claims。另有有界 Confirmation 索引暴露可恢复的
草稿、待运行/运行中 slot 和等待 Evidence 的记录，避免重复创建。
`graph_records` 只含 finalized Intent/Plan locators 和 sequence binding；mutable
草稿已隔离到顶层 `workspace.graph_record_drafts`，并显式标记
`mutable_non_authoritative`。另一个有界、确定性的 `occurrences` locator 主动暴露
failed/interrupted/incomplete Run、缺失声明输出、Evidence-ineligible attempt，以及
finalized 但尚未进入 Claim disposition 的 Evidence；它只记录事实和来源，不推断
失败原因。Agent 再按当前问题或 ID 下钻，不会默认
重读全部历史或把完整 Claims 复制进启动上下文。

终态 Claim 不会因压缩而只剩不可逆哈希：Checkpoint finalization 首次封存时会在
...
ExperimentIntent/ControlGraphSpec、Run/Observation/Evidence/Cohort、Confirmation 工作、work、正式制品和宿主变更路径只保留有限 locator
...
Checkpoint v5 还 pin finalization 当时的 graph-record-sequence locator，作为可
追溯的历史边界，而不是从 Checkpoint 反向生成图记录或 Claim 支持。
Checkpoint 只保留代表性失败 Run 的精确引用；失败原因仍是候选解释，可复用经验
仍须进入有范围、有 Evidence 的 Claim。
...
记录为 absent 并保留；它使该 Run 不能进入 Evidence，但不会删除 Run、日志、
预算承诺或发生史。若文件稍后出现，或已有输出无法建立稳定哈希，后续 Run 将
fail-closed，防止存储预算被绕过。Run 只证明尝试及其结果状态；原因分析是候选
解释，跨情形复用的“经验”必须由 Claim-specific Evidence 支持。

升级前已经存在的连续 V1/V2 Run 历史不会被普通 `run` 自动重建 ledger。
先用外部记录确认历史完整，再显式执行
`python -m tools.studyctl ledger-migrate SC-NNNN`；迁移拒绝空历史、ID 缺口、
任何 V3+ Run 或已有 ledger。V4 是首个显式记录 exploratory/confirmatory 角色的
Manifest 版本；V5 新增可为 `null` 的精确 control-node binding。V1–V3 永久按
exploratory 解释，活动 PLAN 也不会给未声明 `--plan-node` 的 Run 补绑定。
